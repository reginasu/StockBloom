...
def render_table(frame):
    ...
    display_columns = {
        ...
        "BB_Width": "布林寬度",
        "PullbackSignal": "回檔訊號",
        "FinalPullbackSignal": "🎯外資連買回檔",
        "PreBreakoutSignal": "突破前兆",
    }
    # 確保所需欄位皆存在
...
def table_config():
    return {
        "股票代號": st.column_config.LinkColumn(
            "股票代號", display_text=r".*/quote/([^/]+)/technical-analysis"
        ),
        "股票名稱": st.column_config.LinkColumn(
            "股票名稱", display_text=r".*/profile\?name=(.*)"
        ),
        ...
        "回檔訊號": st.column_config.CheckboxColumn("回檔訊號"),
        "🎯外資連買回檔": st.column_config.CheckboxColumn("🎯外資連買回檔"),
        "突破前兆": st.column_config.CheckboxColumn("突破前兆"),
    }
        ...
        try:
            ...
            for row in data.itertuples():
                ...
                rows.append({
                    ...
                    "Name": (
                        ""
                        if str(row.Name).lower() in {"nan", "none"}
...
def build_candidates(quotes):
    volume_pool = sorted(
        quotes,
        key=lambda ticker: (
            quotes[ticker]["Volume"]
            if pd.notna(quotes[ticker]["Volume"])
            else -1
        ),
        reverse=True,
    )[:TOP_VOLUME_LIMIT]
    ...
    if quote and len(df) > 0:
        ...
        if pd.notna(quote_date) and pd.notna(latest_date):
            if quote_date > latest_date:
                df.loc[quote_date, required] = [
                    quote.get(column) for column in required
                ]
                df = df.sort_index()
...
def calculate_metrics(ticker, quote):
    try:
        ...
        prior_120 = (
            bb_values[-121:-1] if len(bb_values) >= 121 else bb_values[:-1]
        )
        ...
        return {
            ...
            "BB_Width_60dNewLow": is_bb_width_new_low(
                prior_60, current_bb_width, 60
            ),
            "BB_Width_120dNewLow": is_bb_width_new_low(
                prior_120, current_bb_width, 120
            ),
        }
    except Exception:
        return None
...
def evaluate_signals(metrics, price_pool, volume_pool):
    ...
    support_near = is_support_near_ma(
        close, metrics["MA5"]
    ) or is_support_near_ma(close, metrics["MA10"])

    pullback_optimized = (
        ma_trend and close >= open_price and support_near and metrics["K"] <= 70
    )
    pullback_100_250 = (
    ...
    bb_width_new_low = metrics.get(
        "BB_Width_60dNewLow", False
...
def foreign_buy_two_days(stock_id):
    ...
    try:
        ...
        foreign_df["buy"] = pd.to_numeric(
            foreign_df["buy"], errors="coerce"
        ).fillna(0)
        foreign_df["sell"] = pd.to_numeric(
            foreign_df["sell"], errors="coerce"
        ).fillna(0)
        foreign_df["net_buy"] = foreign_df["buy"] - foreign_df["sell"]

        daily_net = (
            foreign_df.groupby("date")["net_buy"].sum().sort_index(ascending=False)
        )
        ...
        return bool(
            daily_net.iloc[0] > 0
            and daily_net.iloc[1] > 0
            and daily_net.head(2).sum() > 500
        )
...
def save_results_to_supabase(df: pd.DataFrame):
    ...
    for _, row in signal_df.iterrows():
        records.append({
            ...
            "final_pullback_signal": bool(
                row.get("FinalPullbackSignal", False)
            ),
    ...
    try:
        ...
        return (
            True,
            f"成功同步 {len(records)} 筆觸發訊號至 Supabase 資料庫"
            " (已完成格式化與防重複處置)！",
        )
    except Exception as error:
...
def scan_market():
    ...
    with ThreadPoolExecutor(max_workers=SCAN_WORKERS) as executor:
        metric_rows = list(
            executor.map(
                lambda ticker: calculate_metrics(ticker, quotes.get(ticker, {})),
                candidates,
            )
        )
    metric_rows = [row for row in metric_rows if row]
    results = [
        evaluate_signals(row, price_pool, volume_pool) for row in metric_rows
    ]
    ...
    for row in results:
        ...
        if row.get("Pullback100250", False):
            ...
            row["ForeignBuy2Days"] = foreign_buy_two_days(
                row["Ticker"].rsplit(".", 1)[0]
            )
        row["FinalPullbackSignal"] = (
            row.get("Pullback100250", False) and row["ForeignBuy2Days"]
        )
...
def main():
    ...
    with st.sidebar:
        ...
        scan_requested = st.button(
            "🚀 重新掃描市場並記錄", type="primary", use_container_width=True
        )
        st.divider()
    ...
    if scan_requested or "scan_result" not in st.session_state:
        with st.spinner(
            "正在抓取最新市場行情、計算策略技術指標並同步至 Supabase..."
        ):
            st.session_state.scan_result = scan_market()

    output, candidate_count, price_count, volume_count = (
        st.session_state.scan_result
    )

    if output is None or output.empty:
        st.warning(
            "⚠️ 目前無有效市場資料或未掃描到符合條件的標的，請點擊「重新掃描市場」。"
        )
        return
    ...
    signal_mask = (
        ...
        | output.get(
            "FinalPullbackSignal", pd.Series(False, index=output.index)
        )
    )
    ...
    metric_columns[3].metric(
        "100~250 回檔", f"{int(output['Pullback100250'].sum())} 檔"
    )
    metric_columns[4].metric(
        "🎯 外資連買回檔", f"{int(output['FinalPullbackSignal'].sum())} 檔"
    )
    metric_columns[5].metric(
        "突破前兆", f"{int(output['PreBreakoutSignal'].sum())} 檔"
    )
    ...
    with tabs[0]:
        st.subheader("🎯 外資連買回檔標的 (FinalPullbackSignal)")
        final_pullback_df = output[output["FinalPullbackSignal"]].sort_values(
            "Close", ascending=False
        )
        if not final_pullback_df.empty:
            st.dataframe(
                render_table(final_pullback_df),
                column_config=table_config(),
                width="stretch",
                hide_index=True,
            )
        else:
            st.info("目前無符合外資連買回檔條件的標的。")
    ...
    with tabs[1]:
        ...
        if not output[signal_mask].empty:
            st.dataframe(
                render_table(
                    output[signal_mask].sort_values("Close", ascending=False)
                ),
                column_config=table_config(),
                width="stretch",
                hide_index=True,
    ...
    with tabs[2]:
        st.subheader("均線多頭回檔")
        pullback = output[
            output["FinalPullbackSignal"] | output["PullbackSignal"]
        ].sort_values("K")
        if not pullback.empty:
            st.dataframe(
                render_table(pullback),
                column_config=table_config(),
                width="stretch",
                hide_index=True,
            )
